...
describe("Markdown navigation migrated from Organon", () => {
  ...
  it("preserves opaque IDs and full, tree, section, threshold, and truncation modes", () => {
    ...
    expect(() => renderMarkdown(source, false, "missing", false, 5000)).toThrow(
      'section "missing" not found',
    );
    expect(renderMarkdown("plain content", true, undefined, false, 1)).toEqual({
    ...
    expect(
      Array.from(bounded.content.split("\n[content truncated")[0]!).length,
    ).toBe(30_000);
