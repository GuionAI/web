...
describe("Sourcegraph public GraphQL search migrated from Organon", () => {
  it("sends the established GraphQL request and preserves result, file, and line-context ordering", async () => {
    ...
    expect(JSON.parse(String(init?.body))).toEqual({
      query: GRAPHQL_QUERY,
      variables: { query: "repo:example needle" },
    });
  ...
  it("applies the established defaults and count cap before formatting", async () => {
    ...
    const fixture = {
      data: {
        search: {
          results: { matchCount: 21, resultCount: 21, results: manyResults },
        },
      },
    };

    const defaults = await sgraphSearch({
      query: "needle",
      context: 0,
    ...
    const capped = await sgraphSearch({
      query: "needle",
      count: 25,
      fetch: async () => response(fixture),
    });
    ...
    const limited = await sgraphSearch({
      query: "needle",
      count: 1,
      fetch: async () => response(resultsFixture),
    });
  ...
  it("rejects empty queries and malformed, HTTP, and oversized responses without remote-body disclosure", async () => {
    ...
    ).rejects.toThrow("invalid response format: missing search field");
    await expect(
      sgraphSearch({
        query: "needle",
        fetch: async () => new Response("not json"),
      }),
    ).rejects.toThrow("sourcegraph search: invalid JSON response");
    await expect(
      sgraphSearch({
        query: "needle",
        fetch: async () =>
      ...
      sgraphSearch({
        ...
        fetch: async () =>
          response({
            data: {
              search: {
                results: { matchCount: 0, resultCount: 0, results: [] },
              },
            },
          }),
      }),
