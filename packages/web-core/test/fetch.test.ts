...
async function withTempCache<T>(
  ...
  const cacheDirectory = mkdtempSync(join(tmpdir(), "guionai-web-fetch-cache-"));
  const fetchPage: FetchPage = (input, signal) => fetchWebPage(input, signal, { cacheDirectory });
  try {
...
describe.sequential("browserless fetch migrated from Organon", () => {
  it("validates HTTP URLs and follows redirects", async () => {
    ...
    try {
      await withTempCache(async (fetchPage) => {
        await expect(fetchPage({ url: "file:///tmp/page" })).rejects.toThrow("URL must use http or https");
        await expect(fetchPage({ url: "not a URL" })).rejects.toThrow("invalid URL");
        await expect(fetchPage({ url: `${url}/redirect`, full: true })).resolves.toMatchObject({
  ...
  it("rejects HTTP failures but accepts a final 3xx response", async () => {
    let status = 399;
    const fakeFetch = vi.fn(async () => new Response("status body", { status, headers: { "Content-Type": "text/plain" } })) as typeof fetch;
    await withTempCache(async (_fetchPage, cacheDirectory) => {
      await expect(fetchWebPage({ url: "http://status.test/399" }, undefined, { cacheDirectory, fetch: fakeFetch })).resolves.toMatchObject({ content: "status body" });
      status = 400;
      await expect(fetchWebPage({ url: "http://status.test/400" }, undefined, { cacheDirectory, fetch: fakeFetch })).rejects.toThrow("HTTP 400");
      status = 503;
      await expect(fetchWebPage({ url: "http://status.test/503" }, undefined, { cacheDirectory, fetch: fakeFetch })).rejects.toThrow("HTTP 503");
    });
  });
  ...
  it("returns textual non-HTML bodies and extracts HTML through linkedom and Defuddle", async () => {
    ...
    try {
      await withTempCache(async (fetchPage) => {
        await expect(fetchPage({ url: `${url}/text`, full: true })).resolves.toMatchObject({ content: "package main\n\nfunc main() {}\n" });
        await expect(fetchPage({ url, full: true })).resolves.toEqual({ url, mode: "full", content: "Extracted text.\n" });
      });
    } finally {
      await close(server);
    }
  ...
  it("rejects empty HTML extraction, known binary media, and NUL-bearing bodies", async () => {
    ...
    try {
      await withTempCache(async (fetchPage) => {
        await expect(fetchPage({ url: `${url}/empty` })).rejects.toThrow("no content could be extracted");
        await expect(fetchPage({ url })).rejects.toThrow(/binary content.*curl -L -O/s);
        await expect(fetchPage({ url: `${url}/nul` })).rejects.toThrow(/binary content/);
      });
    } finally {
      await close(server);
    }
    ...
    try {
      await withTempCache(async (fetchPage, cacheDirectory) => {
        ...
        const day = [yesterday.getFullYear(), yesterday.getMonth() + 1, yesterday.getDate()]
          ...
          .join("-");
        renameSync(join(cacheDirectory, file!), join(cacheDirectory, file!.replace(/__[0-9]{4}-[0-9]{2}-[0-9]{2}[.]md$/, `__${day}.md`)));
        await fetchPage({ url: cachedURL });
        expect(requests).toBe(2);

        const windowsSafeURL = `${url}/windows?value=%3C%3E%3A%22%7C%3F*`;
        await fetchPage({ url: windowsSafeURL });
        expect(readdirSync(cacheDirectory).every((name) => /^[0-9a-f]{64}__[0-9-]+[.]md$/.test(name) && name.length <= 80)).toBe(true);
      });

      const cacheFile = join(cacheRoot, "not-a-directory");
      writeFileSync(cacheFile, "cache failure");
      await fetchWebPage({ url: `${url}/uncached` }, undefined, { cacheDirectory: cacheFile });
      await fetchWebPage({ url: `${url}/uncached` }, undefined, { cacheDirectory: cacheFile });
      expect(requests).toBe(5);

      const brokenDirectory = mkdtempSync(join(cacheRoot, "broken-"));
      await fetchWebPage({ url: `${url}/write-failure` }, undefined, { cacheDirectory: brokenDirectory });
  ...
  it("normalizes the 30-second request timeout without waiting", async () => {
    ...
    const timeoutSpy = vi.spyOn(globalThis, "setTimeout").mockImplementation(
      ((handler: (...args: never[]) => void, timeout?: number, ...args: never[]) =>
        realSetTimeout(handler, timeout === 30_000 ? 0 : timeout, ...args)) as unknown as typeof setTimeout,
    );
    try {
      await expect(fetchWebPage({ url: "http://fixture.test/slow" }, undefined, {
        cacheDirectory,
  ...
  it("normalizes cancellation before the request, during transport, and after cache access", async () => {
    ...
    try {
      ...
      await expect(fetchWebPage({ url: "http://fixture.test/page" }, before.signal, { cacheDirectory: cacheDirectories[0], fetch: noNetwork })).rejects.toThrow("Operation aborted");
      expect(noNetwork).not.toHaveBeenCalled();

      const controller = new AbortController();
      let transportSawAbort = false;
      let startTransport: () => void;
      const transportStarted = new Promise<void>((resolve) => { startTransport = resolve; });
      const pending = fetchWebPage({ url: "http://fixture.test/slow" }, controller.signal, {
      ...
      const bodyReadStarted = new Promise<void>((resolve) => { startReading = resolve; });
      let releasePull: () => void;
      const bodyPending = fetchWebPage({ url: "http://fixture.test/body" }, bodyController.signal, {
      ...
      await expect(fetchWebPage({ url: "http://fixture.test/cached" }, cached.signal, {
      ...
      for (const directory of cacheDirectories) rmSync(directory, { recursive: true, force: true });
    }
  });
  ...
  it("keeps client-only SPA script output unrendered", async () => {
    const { server, url } = await startServer((_req, res) => {
      res.setHeader("Content-Type", "text/html");
      res.end("<html><body><main><p>Initial static content.</p></main><script>document.querySelector('main').innerHTML = '<p>SPA_MARKER_RENDERED</p>'</script></body></html>");
    });
    try {
      await withTempCache(async (fetchPage) => {
