...
try {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (target === "https://fixture.test/page") {
      return new Response(
        "<article><h1>Packed page</h1><p>fixture content</p></article>",
        { headers: { "content-type": "text/html" } },
      );
    }
    if (target.startsWith("https://context7.com/api/v1/search?")) {
      if (new URL(target).searchParams.get("query") !== "react")
        return new Response("bad docs query", { status: 400 });
      return Response.json({
        results: [
          {
    ...
    if (
      target ===
      "https://context7.com/api/v1/reactjs/react.dev?type=txt&topic=hooks&tokens=50"
    )
      return new Response("Packed documentation");
    if (target === "https://sourcegraph.com/.api/graphql") {
      if (JSON.parse(init.body).variables.query !== "packed source")
        return new Response("bad source query", { status: 400 });
      return Response.json({
        data: {
  ...
  try {
    ...
    const search = await byName.web_search.execute("test", {
      queries: ["packed search"],
    });
    const fetched = await byName.web_fetch.execute("test", {
      url: "https://fixture.test/page",
      full: true,
    });
    const resolve = await byName.web_docs.execute("test", {
      action: "resolve",
      query: "react",
    });
    const docs = await byName.web_docs.execute("test", {
      action: "fetch",
      library_id: "/reactjs/react.dev",
      topic: "hooks",
      tokens: 50,
    });
    const source = await byName.web_sgraph.execute("test", {
      query: "packed source",
    });
    if (
      search.details.results[0]?.title !== "Packed search" ||
      !fetched.details.content.includes("fixture content") ||
      resolve.details.libraries[0]?.id !== "/reactjs/react.dev" ||
      docs.details.content !== "Packed documentation" ||
      !source.details.content.includes("No results found")
    ) {
      throw new Error(
        "packed extension could not execute all tools against local fixtures",
      );
    }
  } finally {
