...
four Guion-managed compatible copies with the same ids as the official shipped
...
**Guion-Managed DSH Preset**:
A full snapshot under `${DSH_HOME:-$HOME/.dsh}/.agent-presets` generated by
`web dsh sync` from the installed official rc.1 package. Its
`.guion-managed.json` marker records the source version and identity. Sync may
replace only this marked output; an unmarked same-id directory is user-owned
and causes a conflict. `web dsh doctor` verifies that all four snapshots are
current, complete, and free of scoped `tool-web` rows.
_Avoid_: Hand-edited output, shipped preset

**Ordinary DSH User Preset**:
A user-created directory in the DSH user preset root without Guion's marker.
It remains visible when the bundle hides the shipped root and is never
overwritten or deleted by `web dsh sync`.
_Avoid_: Guion-managed preset, shipped preset
